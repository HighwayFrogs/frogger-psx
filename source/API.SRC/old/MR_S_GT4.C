...
*------------------------------------------------------------------------------
*
*	Polygon rendering routines (mesh based), for gouraud textured quads
*
...
*				MR_MPRIM_GT4 (gouraud textured quad) primitives.
*
*	INPUTS		vert_ptr	-	Pointer to vertex block
*				norm_ptr	-	Pointer to normal block
*				prim_ptr	-	Pointer to MR_MPRIM_GT4 block
...
MR_VOID	MRSpecialDisplayMeshPolys_GT4(	MR_SVEC* 		vert_ptr,
							 			MR_SVEC* 		norm_ptr,
										MR_ULONG*		prim_ptr,
										MR_ULONG*		mem_ptr,
										MR_MESH_PARAM*	param_ptr,
										...
										MR_ULONG		flags)
{
	...
	param_ptr->p_v0 = vert_ptr + ((MR_MPRIM_GT4*)prim_ptr)->mp_p0;
	param_ptr->p_v1 = vert_ptr + ((MR_MPRIM_GT4*)prim_ptr)->mp_p1;
	param_ptr->p_v2 = vert_ptr + ((MR_MPRIM_GT4*)prim_ptr)->mp_p2;
	param_ptr->p_v3 = vert_ptr + ((MR_MPRIM_GT4*)prim_ptr)->mp_p3;
	...
	while (prim_count--)
		...
		gte_rtpt();
		param_ptr->p_v0 = vert_ptr + ((MR_MPRIM_GT4*)prim_ptr+1)->mp_p0;	// Fetch next vertices
		param_ptr->p_v1 = vert_ptr + ((MR_MPRIM_GT4*)prim_ptr+1)->mp_p1;	// while in gte_rtpt()
		param_ptr->p_v3 = vert_ptr + ((MR_MPRIM_GT4*)prim_ptr+1)->mp_p3;	// delay slot
		...
		gte_rtps();
		MR_COPY32(work_cvec, ((MR_MPRIM_GT4*)prim_ptr)->mp_cvec);
		...
		param_ptr->p_v2 = vert_ptr + ((MR_MPRIM_GT4*)prim_ptr+1)->mp_p2;// Fetch next vertex in delay slot
			...
			if (
				...
				if (!(flags & MR_MESH_INST_NO_LIGHTING))								// We want lighting?
					...
					if ((flags & MR_MESH_INST_IGNORE_NCLIP) && (flags & MR_MESH_INST_FIX_NCLIP_NORMALS) && (nclip_poly))
						{
						work_svec[0].vx = -(norm_ptr + ((MR_MPRIM_GT4*)prim_ptr)->mp_n0)->vx;					
						work_svec[0].vy = -(norm_ptr + ((MR_MPRIM_GT4*)prim_ptr)->mp_n0)->vy;					
						work_svec[0].vz = -(norm_ptr + ((MR_MPRIM_GT4*)prim_ptr)->mp_n0)->vz;					
						work_svec[1].vx = -(norm_ptr + ((MR_MPRIM_GT4*)prim_ptr)->mp_n1)->vx;					
						work_svec[1].vy = -(norm_ptr + ((MR_MPRIM_GT4*)prim_ptr)->mp_n1)->vy;					
						work_svec[1].vz = -(norm_ptr + ((MR_MPRIM_GT4*)prim_ptr)->mp_n1)->vz;					
						work_svec[2].vx = -(norm_ptr + ((MR_MPRIM_GT4*)prim_ptr)->mp_n2)->vx;					
						work_svec[2].vy = -(norm_ptr + ((MR_MPRIM_GT4*)prim_ptr)->mp_n2)->vy;					
						work_svec[2].vz = -(norm_ptr + ((MR_MPRIM_GT4*)prim_ptr)->mp_n2)->vz;					
						work_svec[3].vx = -(norm_ptr + ((MR_MPRIM_GT4*)prim_ptr)->mp_n3)->vx;					
						work_svec[3].vy = -(norm_ptr + ((MR_MPRIM_GT4*)prim_ptr)->mp_n3)->vy;					
						work_svec[3].vz = -(norm_ptr + ((MR_MPRIM_GT4*)prim_ptr)->mp_n3)->vz;					
						gte_ldv3(&work_svec[0], &work_svec[1], &work_svec[3]);
						}
					else
						{
						gte_ldv3((norm_ptr + ((MR_MPRIM_GT4*)prim_ptr)->mp_n0),
									(norm_ptr + ((MR_MPRIM_GT4*)prim_ptr)->mp_n1),
									(norm_ptr + ((MR_MPRIM_GT4*)prim_ptr)->mp_n3));
						}
					...
					if (light_dpq)
						...
						if ((flags & MR_MESH_INST_IGNORE_NCLIP) && (flags & MR_MESH_INST_FIX_NCLIP_NORMALS) && (nclip_poly))
							...
							{
							gte_ldv0(norm_ptr + ((MR_MPRIM_GT4*)prim_ptr)->mp_n2);
							}
						gte_ncds();
						}
					else
