...
*------------------------------------------------------------------------------
*
*	Polygon rendering routines (mesh based), for gouraud triangles
*
...
/******************************************************************************
*%%%% MRSpecialDisplayMeshPolys_G3
*------------------------------------------------------------------------------
*
*	SYNOPSIS	MR_VOID	MRSpecialDisplayMeshPolys_G3(
*						MR_SVEC*		vert_ptr,
*						MR_SVEC*		norm_ptr,
*						MR_ULONG*		prim_ptr,
*						MR_ULONG*		mem_ptrm
...
*				MR_MPRIM_G3 (gouraud triangle) primitives.
*
*	INPUTS		vert_ptr	-	Pointer to vertex block
*				norm_ptr	-	Pointer to normal block
*				prim_ptr	-	Pointer to MR_MPRIM_G3 block
...
MR_VOID	MRSpecialDisplayMeshPolys_G3(	MR_SVEC*		vert_ptr,
							 			MR_SVEC*		norm_ptr,
							 		 	MR_ULONG*		prim_ptr,
							 		 	MR_ULONG*		mem_ptr,
							 		 	MR_MESH_PARAM* 	param_ptr,
							 			...
							 			MR_ULONG		flags)
{
	...
	param_ptr->p_v0 = vert_ptr + ((MR_MPRIM_G3*)prim_ptr)->mp_p0;
	param_ptr->p_v1 = vert_ptr + ((MR_MPRIM_G3*)prim_ptr)->mp_p1;
	param_ptr->p_v2 = vert_ptr + ((MR_MPRIM_G3*)prim_ptr)->mp_p2;
	...
	while (prim_count--)
		...
		gte_rtpt();
		param_ptr->p_v0 = vert_ptr + ((MR_MPRIM_G3*)prim_ptr+1)->mp_p0;	// Fetch next vertices
		param_ptr->p_v1 = vert_ptr + ((MR_MPRIM_G3*)prim_ptr+1)->mp_p1;	// while in gte_rtpt()
		param_ptr->p_v2 = vert_ptr + ((MR_MPRIM_G3*)prim_ptr+1)->mp_p2;	// delay slot
		
		// Normal clip first three points
		gte_nclip();
		MR_COPY32(work_cvec, ((MR_MPRIM_G3*)prim_ptr)->mp_cvec);
	...
	//---------------
		...
		if (
			...
			{
			gte_stsxy3_g3((POLY_G3*)mem_ptr);

			if (
				(((((POLY_G3*)mem_ptr)->y0 >= 0) ||
				  (((POLY_G3*)mem_ptr)->y1 >= 0) ||
				  (((POLY_G3*)mem_ptr)->y2 >= 0)) &&
				 ((((POLY_G3*)mem_ptr)->y0 < disp_h) ||
				  (((POLY_G3*)mem_ptr)->y1 < disp_h) ||
				  (((POLY_G3*)mem_ptr)->y2 < disp_h)))
				)
				{
				...
				if (!(flags & MR_MESH_INST_NO_LIGHTING))								// We want lighting?
					...
					if ((flags & MR_MESH_INST_IGNORE_NCLIP) && (flags & MR_MESH_INST_FIX_NCLIP_NORMALS) && (param_ptr->p_nclip_result <= 0))
						{
						work_svec[0].vx = -(norm_ptr + ((MR_MPRIM_G3*)prim_ptr)->mp_n0)->vx;					
						work_svec[0].vy = -(norm_ptr + ((MR_MPRIM_G3*)prim_ptr)->mp_n0)->vy;					
						work_svec[0].vz = -(norm_ptr + ((MR_MPRIM_G3*)prim_ptr)->mp_n0)->vz;					
						work_svec[1].vx = -(norm_ptr + ((MR_MPRIM_G3*)prim_ptr)->mp_n1)->vx;					
						work_svec[1].vy = -(norm_ptr + ((MR_MPRIM_G3*)prim_ptr)->mp_n1)->vy;					
						work_svec[1].vz = -(norm_ptr + ((MR_MPRIM_G3*)prim_ptr)->mp_n1)->vz;					
						work_svec[2].vx = -(norm_ptr + ((MR_MPRIM_G3*)prim_ptr)->mp_n2)->vx;					
						work_svec[2].vy = -(norm_ptr + ((MR_MPRIM_G3*)prim_ptr)->mp_n2)->vy;					
						work_svec[2].vz = -(norm_ptr + ((MR_MPRIM_G3*)prim_ptr)->mp_n2)->vz;					
						gte_ldv3(&work_svec[0], &work_svec[1], &work_svec[2]);
						}
					else
						{
						gte_ldv3((norm_ptr + ((MR_MPRIM_G3*)prim_ptr)->mp_n0),
									(norm_ptr + ((MR_MPRIM_G3*)prim_ptr)->mp_n1),
									(norm_ptr + ((MR_MPRIM_G3*)prim_ptr)->mp_n2));
						}
					...
					gte_strgb3_g3((POLY_G3*)mem_ptr);
					}
				else
					{
					MR_COPY32(((POLY_G3*)mem_ptr)->r0, work_cvec);
					MR_COPY32(((POLY_G3*)mem_ptr)->r1, work_cvec);
					MR_COPY32(((POLY_G3*)mem_ptr)->r2, work_cvec);
					}
