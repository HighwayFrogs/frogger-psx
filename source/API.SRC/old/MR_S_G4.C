...
/******************************************************************************
*%%%% MRSpecialDisplayMeshPolys_G4
*------------------------------------------------------------------------------
*
*	SYNOPSIS	MR_VOID	MRSpecialDisplayMeshPolys_G4(
*						MR_SVEC*		vert_ptr,
*						MR_SVEC*		norm_ptr,
*						MR_ULONG*		prim_ptr,
*						MR_ULONG*		mem_ptrm
...
MR_VOID	MRSpecialDisplayMeshPolys_G4(	MR_SVEC*		vert_ptr,
										MR_SVEC*		norm_ptr,
										MR_ULONG*		prim_ptr,
										MR_ULONG*		mem_ptr,
										MR_MESH_PARAM*	param_ptr,
										...
										MR_ULONG		flags)
{
	...
	while (prim_count--)
		...
		gte_rtps();
		MR_COPY32(work_cvec, ((MR_MPRIM_G4*)prim_ptr)->mp_cvec);
			...
			if (
				...
				if (!(flags & MR_MESH_INST_NO_LIGHTING))								// We want lighting?
					...
					if ((flags & MR_MESH_INST_IGNORE_NCLIP) && (flags & MR_MESH_INST_FIX_NCLIP_NORMALS) && (nclip_poly))
						{
						work_svec[0].vx = -(norm_ptr + ((MR_MPRIM_G4*)prim_ptr)->mp_n0)->vx;					
						work_svec[0].vy = -(norm_ptr + ((MR_MPRIM_G4*)prim_ptr)->mp_n0)->vy;					
						work_svec[0].vz = -(norm_ptr + ((MR_MPRIM_G4*)prim_ptr)->mp_n0)->vz;					
						work_svec[1].vx = -(norm_ptr + ((MR_MPRIM_G4*)prim_ptr)->mp_n1)->vx;					
						work_svec[1].vy = -(norm_ptr + ((MR_MPRIM_G4*)prim_ptr)->mp_n1)->vy;					
						work_svec[1].vz = -(norm_ptr + ((MR_MPRIM_G4*)prim_ptr)->mp_n1)->vz;					
						work_svec[2].vx = -(norm_ptr + ((MR_MPRIM_G4*)prim_ptr)->mp_n2)->vx;					
						work_svec[2].vy = -(norm_ptr + ((MR_MPRIM_G4*)prim_ptr)->mp_n2)->vy;					
						work_svec[2].vz = -(norm_ptr + ((MR_MPRIM_G4*)prim_ptr)->mp_n2)->vz;					
						work_svec[3].vx = -(norm_ptr + ((MR_MPRIM_G4*)prim_ptr)->mp_n3)->vx;					
						work_svec[3].vy = -(norm_ptr + ((MR_MPRIM_G4*)prim_ptr)->mp_n3)->vy;					
						work_svec[3].vz = -(norm_ptr + ((MR_MPRIM_G4*)prim_ptr)->mp_n3)->vz;					
					...
					{
					MR_COPY32(((POLY_G4*)mem_ptr)->r0, work_cvec);
					MR_COPY32(((POLY_G4*)mem_ptr)->r1, work_cvec);
					MR_COPY32(((POLY_G4*)mem_ptr)->r2, work_cvec);
					MR_COPY32(((POLY_G4*)mem_ptr)->r3, work_cvec);
					}
				}
			}
