...
*				MR_MPRIM_F3 (flat shaded triangle) primitives.
*
...
MR_VOID	MRDisplayMeshPolys_F3(	MR_SVEC* 		vert_ptr,
							  	MR_SVEC* 		norm_ptr,
							  	MR_ULONG*		prim_ptr,
							  	MR_ULONG*		mem_ptr,
							  	MR_MESH_PARAM*	param_ptr,
							 	MR_BOOL 		light_dpq)
{
	...
	while (prim_count--)
		...
		gte_nclip();
		gte_ldrgb(&((MR_MPRIM_F3*)prim_ptr)->mp_cvec);							// Load RGB in delay slot
