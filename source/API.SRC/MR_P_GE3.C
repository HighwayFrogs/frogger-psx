...
*------------------------------------------------------------------------------
*
*	Polygon rendering routines (mesh based), gouraud shaded environment mapped
*	triangles
*
...
/******************************************************************************
*%%%% MRDisplayMeshPolys_GE3
*------------------------------------------------------------------------------
*
*	SYNOPSIS	MR_VOID	MRDisplayMeshPolys_GE3(
*						MR_SVEC*		vert_ptr,
*						MR_SVEC*		norm_ptr,
*						MR_ULONG*		prim_ptr,
*						MR_ULONG*		mem_ptrm
...
MR_VOID	MRDisplayMeshPolys_GE3(	MR_SVEC*		vert_ptr,
								MR_SVEC*		norm_ptr,
								MR_ULONG*		prim_ptr,
							 	MR_ULONG*		mem_ptr,
							 	MR_MESH_PARAM*	param_ptr,
							 	MR_BOOL			light_dpq)
{
	...
	while (prim_count--)
		...
		gte_nclip();
		gte_ldrgb(&((MR_MPRIM_GE3*)prim_ptr)->mp_cvec);						// Load poly-variable RGB in delay slot
...
//				gte_ApplyRotMatrix(norm_ptr + (((MR_MPRIM_GE3*)prim_ptr)->mp_en0), &work_vector);
//				MR_SET16(((POLY_GT3*)mem_ptr)->u0, (work_vector.vx >> 6) + uofs + (((-work_vector.vy >> 6) + vofs) << 8));
//
//				gte_ApplyRotMatrix(norm_ptr + (((MR_MPRIM_GE3*)prim_ptr)->mp_en1), &work_vector);
//				MR_SET16(((POLY_GT3*)mem_ptr)->u1, (work_vector.vx >> 6) + uofs + (((-work_vector.vy >> 6) + vofs) << 8));
//
//				gte_ApplyRotMatrix(norm_ptr + (((MR_MPRIM_GE3*)prim_ptr)->mp_en2), &work_vector);
