...
*------------------------------------------------------------------------------
*
*	Polygon rendering routines (mesh based), for flat textured quads
*
...
/******************************************************************************
*%%%% MRSpecialDisplayMeshPolys_FT4
*------------------------------------------------------------------------------
*
*	SYNOPSIS	MR_VOID	MRSpecialDisplayMeshPolys_FT4(
*						MR_SVEC*  		vert_ptr,
*						MR_SVEC*  		norm_ptr,
*						MR_ULONG*		prim_ptr,
*						MR_ULONG*		mem_ptrm
...
*				MR_MPRIM_FT4 (flat textured quad) primitives.
*
*	INPUTS		vert_ptr	-	Pointer to vertex block
*				norm_ptr	-	Pointer to normal block
*				prim_ptr	-	Pointer to MR_MPRIM_FT4 block
...
MR_VOID	MRSpecialDisplayMeshPolys_FT4(	MR_SVEC*	 	vert_ptr,
										MR_SVEC*	 	norm_ptr,
										MR_ULONG*		prim_ptr,
									 	MR_ULONG*		mem_ptr,
									 	MR_MESH_PARAM*	param_ptr,
...
{						
	...
	param_ptr->p_v0 = vert_ptr + ((MR_MPRIM_FT4*)prim_ptr)->mp_p0;
	param_ptr->p_v1 = vert_ptr + ((MR_MPRIM_FT4*)prim_ptr)->mp_p1;
	param_ptr->p_v2 = vert_ptr + ((MR_MPRIM_FT4*)prim_ptr)->mp_p2;
	param_ptr->p_v3 = vert_ptr + ((MR_MPRIM_FT4*)prim_ptr)->mp_p3;
	...
	while (prim_count--)
		...
		gte_rtpt();
		param_ptr->p_v0 = vert_ptr + ((MR_MPRIM_FT4*)prim_ptr+1)->mp_p0;	// Fetch next vertices
		param_ptr->p_v1 = vert_ptr + ((MR_MPRIM_FT4*)prim_ptr+1)->mp_p1;	// while in gte_rtpt()
		param_ptr->p_v3 = vert_ptr + ((MR_MPRIM_FT4*)prim_ptr+1)->mp_p3;	// delay slot
		
		// Normal clip first three points
		gte_nclip();
		nclip_poly = FALSE;															
		gte_stsxy0((MR_LONG*)&((POLY_FT4*)mem_ptr)->x0);					// Store sxy's from rtpt() in delay slot
		...
		gte_rtps();
		param_ptr->p_v2 = vert_ptr + ((MR_MPRIM_FT4*)prim_ptr+1)->mp_p2;// Fetch next vertex in delay slot
		MR_COPY32(work_cvec, ((MR_MPRIM_FT4*)prim_ptr)->mp_cvec);
			...
			if (
				...
				if (!(flags & MR_MESH_INST_NO_LIGHTING))								// We want lighting?
					...
					if ((flags & MR_MESH_INST_IGNORE_NCLIP) && (flags & MR_MESH_INST_FIX_NCLIP_NORMALS) && (nclip_poly))
						{
						work_svec.vx = -(norm_ptr + ((MR_MPRIM_FT4*)prim_ptr)->mp_n0)->vx;					
						work_svec.vy = -(norm_ptr + ((MR_MPRIM_FT4*)prim_ptr)->mp_n0)->vy;					
						work_svec.vz = -(norm_ptr + ((MR_MPRIM_FT4*)prim_ptr)->mp_n0)->vz;					
						gte_ldv0(&work_svec);
						}
					else
						{
						gte_ldv0(norm_ptr + ((MR_MPRIM_FT4*)prim_ptr)->mp_n0);
						}
