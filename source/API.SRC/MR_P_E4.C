...
*------------------------------------------------------------------------------
*
*	Polygon rendering routines (mesh based), for environment mapped quads
*
...
*				MR_MPRIM_E4 (environment mapped quad) primitives.
*
*	INPUTS		vert_ptr	-	Pointer to vertex block
*				norm_ptr	-	Pointer to normal block
*				prim_ptr	-	Pointer to MR_MPRIM_E4 block
...
MR_VOID	MRDisplayMeshPolys_E4(	MR_SVEC* 	  	vert_ptr,
								MR_SVEC* 	  	norm_ptr,
							 	MR_ULONG*	  	prim_ptr,
							 	MR_ULONG*	  	mem_ptr,
							 	MR_MESH_PARAM*	param_ptr,
							 	MR_BOOL 		light_dpq)
{
	...
	param_ptr->p_v0 = vert_ptr + ((MR_MPRIM_E4*)prim_ptr)->mp_p0;
	param_ptr->p_v1 = vert_ptr + ((MR_MPRIM_E4*)prim_ptr)->mp_p1;
	param_ptr->p_v2 = vert_ptr + ((MR_MPRIM_E4*)prim_ptr)->mp_p2;
	param_ptr->p_v3 = vert_ptr + ((MR_MPRIM_E4*)prim_ptr)->mp_p3;
	...
	while (prim_count--)
		...
		gte_rtpt();
		param_ptr->p_v0 = vert_ptr + ((MR_MPRIM_E4*)prim_ptr+1)->mp_p0;	// Fetch next vertices
		param_ptr->p_v1 = vert_ptr + ((MR_MPRIM_E4*)prim_ptr+1)->mp_p1;	// while in gte_rtpt()
		param_ptr->p_v3 = vert_ptr + ((MR_MPRIM_E4*)prim_ptr+1)->mp_p3;	// delay slot
		...
		gte_stsxy0((MR_LONG*)&((POLY_FT4*)mem_ptr)->x0);

		// Rotate the fourth point (and perform operations in delay slot)
		gte_rtps();
		gte_ldrgb(&((MR_MPRIM_E4*)prim_ptr)->mp_cvec);						// Load poly-variable RGB in delay slot
		param_ptr->p_v2 = vert_ptr + ((MR_MPRIM_E4*)prim_ptr+1)->mp_p2;		// Fetch next vertex in delay slot
	...
	visible_poly:
		...
		if (
			...
			{
			gte_stsxy3(	(MR_LONG*)&((POLY_FT4*)mem_ptr)->x1,
							(MR_LONG*)&((POLY_FT4*)mem_ptr)->x2,
							(MR_LONG*)&((POLY_FT4*)mem_ptr)->x3);
	
			if (
				(((((POLY_FT4*)mem_ptr)->y0 >= 0) ||
				  (((POLY_FT4*)mem_ptr)->y1 >= 0) ||
				  (((POLY_FT4*)mem_ptr)->y2 >= 0) ||
				  (((POLY_FT4*)mem_ptr)->y3 >= 0)) &&
				 ((((POLY_FT4*)mem_ptr)->y0 < disp_h) ||
				  (((POLY_FT4*)mem_ptr)->y1 < disp_h) ||
				  (((POLY_FT4*)mem_ptr)->y2 < disp_h) ||
				  (((POLY_FT4*)mem_ptr)->y3 < disp_h)))
				)
				{
...
//
//				gte_ApplyRotMatrix(norm_ptr + (((MR_MPRIM_E4*)prim_ptr)->mp_en0), &work_vector);
//				MR_SET16(((POLY_FT4*)mem_ptr)->u0, (work_vector.vx >> 6) + uofs + (((-work_vector.vy >> 6) + vofs) << 8));
//
//				gte_ApplyRotMatrix(norm_ptr + (((MR_MPRIM_E4*)prim_ptr)->mp_en1), &work_vector);
//				MR_SET16(((POLY_FT4*)mem_ptr)->u1, (work_vector.vx >> 6) + uofs + (((-work_vector.vy >> 6) + vofs) << 8));
//
//				gte_ApplyRotMatrix(norm_ptr + (((MR_MPRIM_E4*)prim_ptr)->mp_en2), &work_vector);
//				MR_SET16(((POLY_FT4*)mem_ptr)->u3, (work_vector.vx >> 6) + uofs + (((-work_vector.vy >> 6) + vofs) << 8));
//
//				gte_ApplyRotMatrix(norm_ptr + (((MR_MPRIM_E4*)prim_ptr)->mp_en3), &work_vector);
//				MR_SET16(((POLY_FT4*)mem_ptr)->u2, (work_vector.vx >> 6) + uofs + (((-work_vector.vy >> 6) + vofs) << 8));
//
// --- End of code

				param_ptr->p_n0 = norm_ptr + (((MR_MPRIM_E4*)prim_ptr)->mp_en0);	// Calc en0 address
				gte_ldv0(param_ptr->p_n0);										 	// Load into V0(GTE)
				gte_rtv0();														 	// Rotate en0
				param_ptr->p_n0 = norm_ptr + (((MR_MPRIM_E4*)prim_ptr)->mp_en1);	// Precalc en1 address (delay slot)
				gte_stlvnl(&work_vector);										 	// Store output

				gte_ldv0(param_ptr->p_n0);															
				gte_rtv0();																				// Rotate en1
				MR_SET16(((POLY_FT4*)mem_ptr)->u0, (work_vector.vx >> 6) + uofs + (((-work_vector.vy >> 6) + vofs) << 8));
				param_ptr->p_n0 = norm_ptr + (((MR_MPRIM_E4*)prim_ptr)->mp_en2);
				gte_stlvnl(&work_vector);

				gte_ldv0(param_ptr->p_n0);
				gte_rtv0();					 															// Rotate en2
				MR_SET16(((POLY_FT4*)mem_ptr)->u1, (work_vector.vx >> 6) + uofs + (((-work_vector.vy >> 6) + vofs) << 8));
				param_ptr->p_n0 = norm_ptr + (((MR_MPRIM_E4*)prim_ptr)->mp_en3);
				gte_stlvnl(&work_vector);				
				...
				gte_ldv0(norm_ptr + ((MR_MPRIM_E4*)prim_ptr)->mp_n0);
				...
				gte_strgb((MR_CVEC*)&((POLY_FT4*)mem_ptr)->r0);												
				}
			}
