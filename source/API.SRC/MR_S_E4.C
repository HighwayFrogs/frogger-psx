...
/******************************************************************************
*%%%% MRSpecialDisplayMeshPolys_E4
*------------------------------------------------------------------------------
*
*	SYNOPSIS	MR_VOID	MRSpecialDisplayMeshPolys_E4(
*						MR_SVEC*		vert_ptr,
*						MR_SVEC*		norm_ptr,
*						MR_ULONG*		prim_ptr,
*						MR_ULONG*		mem_ptrm
...
MR_VOID	MRSpecialDisplayMeshPolys_E4(	MR_SVEC* 	  	vert_ptr,
										MR_SVEC* 	  	norm_ptr,
									 	MR_ULONG*	  	prim_ptr,
									 	MR_ULONG*	  	mem_ptr,
									 	MR_MESH_PARAM*	param_ptr,
										...
										MR_ULONG   		flags)
{
	...
	while (prim_count--)
		...
		gte_rtps();
		MR_COPY32(work_cvec, ((MR_MPRIM_E4*)prim_ptr)->mp_cvec);
			...
			if (
				...
				if (!(flags & MR_MESH_INST_NO_LIGHTING))								// We want lighting?
					...
					if ((flags & MR_MESH_INST_IGNORE_NCLIP) && (flags & MR_MESH_INST_FIX_NCLIP_NORMALS) && (nclip_poly))
						{
						work_svec.vx = -(norm_ptr + ((MR_MPRIM_E4*)prim_ptr)->mp_n0)->vx;					
						work_svec.vy = -(norm_ptr + ((MR_MPRIM_E4*)prim_ptr)->mp_n0)->vy;					
						work_svec.vz = -(norm_ptr + ((MR_MPRIM_E4*)prim_ptr)->mp_n0)->vz;					
						gte_ldv0(&work_svec);
						}
					else
						{
					...
					{
					MR_COPY32(((POLY_FT4*)mem_ptr)->r0, work_cvec);
					}				
