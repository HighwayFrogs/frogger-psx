...
;/*------------------------------------------------------------------------------
;/*
;/*	Polygon rendering routines for flat shaded quadrilateral groups
;/*
...
;/*%%%**************************************************************************/
		...
		xdef	MRDisplayMeshPolys_F4
 
		include	"mr_m_hdr.i"	

;/******************************************************************************
;/*%%%% MRDisplayMeshPolys_F4
;/*------------------------------------------------------------------------------
;/*
;/*	SYNOPSIS	MR_VOID	MRDisplayMeshPolys_F4(
;/*				MR_SVEC*	vert_ptr,
;/*				MR_SVEC*	norm_ptr,
;/*				MR_ULONG*	prim_ptr,
;/*				MR_ULONG*	mem_ptr,
...
;/*			MR_MPRIM_F4 primitives (flat shaded quadrilaterals).
;/*
;/*	INPUTS		vert_ptr	-    (a0) Pointer to vertex block
;/*			norm_ptr	-    (a1) Pointer to normal block
;/*			prim_ptr	-    (a2) Pointer to MR_MPRIM_F4 block	
...
.calculate_addprim_masks:				
		...
		ori	s6,s6,$ffff			
		lui	s7,primsize_PF4<<8		; s7 = primitive packet length in upper 8 bits

.calculate_model_primitive_count:
		lw	s8,MP_prims(v0)
							
.precalculate_first_vertices:				
		lh	t0,MPF4_p0(a2)			; Fetch mp_p0
		lh	t1,MPF4_p1(a2)			; Fetch mp_p1
		lh	t2,MPF4_p2(a2)			; Fetch mp_p2
		lh	t3,MPF4_p3(a2)			; Fetch mp_p3
...
.process_next_polygon					
		...
		add	t8,a2,sizeof_MPF4		; //DELAY: t8 points to next MR_MPRIM_F4 structure
		nop
		RTPT					; Rotate 3 vertices
							
.precalculate_next_vertices:				
						
		lh	t0,MPF4_p0(t8)			; Fetch next mp_p0
		lh	t1,MPF4_p1(t8)			; Fetch next mp_p1
		lh	t3,MPF4_p3(t8)			; Fetch next mp_p3
...
.normal_clip:						
		...
		swc2	C2_SXY0,PF4_x0(a3)		; Store XY0, as it will be pushed out of fifo by vertex 2
;		nop					; NOPS not needed as there are 2 instructions between load and RTPS				
;		nop
		RTPS					; Rotate vertex 2

		lwc2	C2_RGB,MPF4_cvec(a2)		; //DELAY: Load RGB in RTPS delay slots

		lh	t2,MPF4_p2(t8)			; Fetch next mp_p2
		nop
		sll	t2,t2,3				; Turn mp_p2 index into SVECTOR offset
...
.process_poly:
		...
		lh	t4,MPF4_n0(a2)			; //DELAY: Fetch mp_n0
...
.clip_polygon:		
		...
		swc2	C2_SXY0,PF4_x1(a3)		; Store XY coordinates for each remaining vertex
		swc2	C2_SXY1,PF4_x2(a3)
		swc2	C2_SXY2,PF4_x3(a3)
...
.add_polygon:
		...
		sw	t9,0(t8)			; Point OT entry to our current POLY_F4
		or	at,at,s7			
		sw	at,0(t9)
		opt	at+

		swc2	C2_RGB2,PF4_rgb(a3)		; Store RGB							
.next_poly:						
		addi	s5,s5,-1			; Decrement primitive count
		addi	a2,a2,sizeof_MPF4		; Point to next MR_MPRIM_F4
		addi	a3,a3,sizeof_PF4		; Point to next POLY_F4
