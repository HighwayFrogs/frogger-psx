...
#/*------------------------------------------------------------------------------
#/*
#/*	Polygon rendering routines for gouraud textured triangle groups
#/*
...
#/*			MR_MPRIM_GT3 primitives (gouraud shaded triangles).
#/*
#/*	INPUTS		vert_ptr	-    (a0) Pointer to vertex block
#/*			norm_ptr	-    (a1) Pointer to normal block
#/*			prim_ptr	-    (a2) Pointer to MR_MPRIM_GT3 block	
...
#/*			in mr_p_gt3.c, with the exception that this doesn't clip
...
.Lprecalculate_first_vertices:
	lh         $t0, MPGT3_p0($a2) # Fetch mp_p0
	lh         $t1, MPGT3_p1($a2) # Fetch mp_p1
	lh         $t2, MPGT3_p2($a2) # Fetch mp_p2
...
.Lprocess_next_polygon:
	...
	addi       $t8, $a2, sizeof_MPGT3 # DELAY: t8 points to next MR_MPRIM_GT3 structure
	nop
	RTPT       # Rotate 3 vertices

.Lprecalculate_next_vertices:
	lh         $t0, MPGT3_p0($t8) # Fetch next mp_p0
	lh         $t1, MPGT3_p1($t8) # Fetch next mp_p1
	lh         $t2, MPGT3_p2($t8) # Fetch next mp_p2
...
.Lnormal_clip:
	...
	lwc2       C2_RGB, MPGT3_cvec($a2) # DELAY: Load RGB while in NCLIP delay slot
...
.Lprocess_poly:
	...
	lh         $t4, MPGT3_n0($a2) # DELAY: EnvMap: Fetch mp_n0
...
.Lclip_polygon:
	...
	lh         $t5, MPGT3_n1($a2) # DELAY: Fetch mp_n1 (always executed in branch slot)
	lh         $t6, MPGT3_n2($a2) # Fetch mp_n2

	swc2       C2_SXY0, PGT3_x0($a3) # Store XY coordinates for each remaining vertex
	swc2       C2_SXY1, PGT3_x1($a3)
	swc2       C2_SXY2, PGT3_x2($a3)
	...
	lwc2       C2_VXY2, SVEC_vx($t6) # Load vector 2 from normal 2
...
.Lnext_poly:
	addi       $a2, $a2, sizeof_MPGT3 # Point to next MR_MPRIM_GT3
