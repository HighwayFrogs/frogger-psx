...
#/******************************************************************************
#/*%%%% MRDisplayMeshPolys_F3
#/*------------------------------------------------------------------------------
#/*
#/*	SYNOPSIS	MR_VOID	MRDisplayMeshPolys_F3(
#/*				MR_SVEC*	vert_ptr,
#/*				MR_SVEC*	norm_ptr,
#/*				MR_ULONG*	prim_ptr,
#/*				MR_ULONG*	mem_ptr,
...
#/*			MR_MPRIM_F3 primitives (flat shaded triangles).
#/*
#/*	INPUTS		vert_ptr	-    (a0) Pointer to vertex block
#/*			norm_ptr	-    (a1) Pointer to normal block
#/*			prim_ptr	-    (a2) Pointer to MR_MPRIM_F3 block	
...
.Lcalculate_addprim_masks:
	...
	lui        $s7, primsize_PF3 << 8 # s7 = primitive packet length in upper 8 bits
...
.Lprocess_next_polygon:
	...
	addi       $t8, $a2, sizeof_MPF3 # DELAY: t8 points to next MR_MPRIM_F3 structure
	nop
...
.Lnormal_clip:
	...
	lwc2       C2_RGB, MPF3_cvec($a2) # DELAY: Load RGB while in NCLIP delay slot
...
.Lprocess_poly:
	...
	lh         $t4, MPF3_n0($a2) # DELAY: Fetch mp_n0
...
.Lclip_polygon:
	...
	nop
	swc2       C2_SXY0, PF3_x0($a3) # Store XY coordinates for each vertex
	swc2       C2_SXY1, PF3_x1($a3)
	swc2       C2_SXY2, PF3_x2($a3)
...
.Ladd_polygon:
	...
	sw         $t9, 0($t8) # Point OT entry to our current POLY_F3
	or         $at, $at, $s7
	sw         $at, 0($t9)

	swc2       C2_RGB2, PF3_rgb($a3) # Store RGB

.Lnext_poly:
	addi       $a2, $a2, sizeof_MPF3 # Point to next MR_MPRIM_F3
	addi       $a3, $a3, sizeof_PF3 # Point to next POLY_F3
