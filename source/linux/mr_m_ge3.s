...
#/*------------------------------------------------------------------------------
#/*
#/*	Polygon rendering routines for gouraud/environment mapped triangle groups
#/*
...
#/*			MR_MPRIM_GE3 primitives (gouraud/environment mapped triangles).
#/*
...
#/*			in mr_p_ge3.c, with the exception that this doesn't clip
...
.Lcalculate_addprim_masks:
	...
	lui        $s7, primsize_PGT3 << 8 # s7 = primitive packet length in upper 8 bits
...
.Lprocess_next_polygon:
	...
	addi       $t8, $a2, sizeof_MPGE3 # DELAY: t8 points to next MR_MPRIM_GE3 structure
	nop
...
.Lnormal_clip:
	...
	lwc2       C2_RGB, MPGE3_cvec($a2) # DELAY: Load RGB while in NCLIP delay slot
...
.Lprocess_poly:
	...
	lh         $t4, MPGE3_en0($a2) # DELAY: EnvMap: Fetch mp_en0
...
.Lclip_polygon:
	...
	swc2       C2_SXY0, PGT4_x0($a3) # Store XY coordinates for each remaining vertex
	swc2       C2_SXY1, PGT4_x1($a3)
	swc2       C2_SXY2, PGT4_x2($a3)
	...
	lh         $t4, MPGE3_en1($a2) # DELAY: Fetch mp_en1
	nop        # DELAY:
	...
	sh         $t5, PGT3_u0($a3) # Store new u0/v0 in polygon (low 16-bits of t5)

	# Handle normal 1
	MVMVA      1, 0, 1, 3, 0 # Equivalent of a RotTrans of vector 1

	lh         $t4, MPGE3_en2($a2) # DELAY: Fetch mp_en2
	nop        # DELAY:
	...
	sll        $t6, $t6, 8 # Shift into bits 8-15
	add        $t5, $t6, $t5 # Now t5 = ((vx>>6)+uofs) + ((-vy>>6)+vofs)<<8))
	sh         $t5, PGT3_u1($a3) # Store new u1/v1 in polygon (low 16-bits of t5)

	# Handle normal 2
	MVMVA      1, 0, 2, 3, 0 # Equivalent of a RotTrans of vector 2

	lh         $t4, MPGE3_n0($a2) # DELAY: Fetch mp_n0 (for lighting)
	nop        # DELAY:
	...
	sll        $t6, $t6, 8 # Shift into bits 8-15
	add        $t5, $t6, $t5 # Now t5 = ((vx>>6)+uofs) + ((-vy>>6)+vofs)<<8))
	sh         $t5, PGT3_u2($a3) # Store new u2/v2 in polygon (low 16-bits of t5)
	...
	lh         $t5, MPGE3_n1($a2) # Fetch mp_n1
	lh         $t6, MPGE3_n2($a2) # Fetch mp_n2
	...
	lwc2       C2_VXY2, SVEC_vx($t6) # Load vector 2 from normal 3
	lwc2       C2_VZ2, SVEC_vz($t6)
...
.Ladd_polygon: # At this point t8 must be the OT position.
	...
	sw         $t9, 0($t8) # Point OT entry to our current POLY_GT3
	or         $at, $at, $s7
	sw         $at, 0($t9)

	swc2       C2_RGB0, PGT3_rgb0($a3) # Store RGB 0
	swc2       C2_RGB1, PGT3_rgb1($a3) # Store RGB 1
	swc2       C2_RGB2, PGT3_rgb2($a3) # Store RGB 2

.Lnext_poly:
	addi       $a2, $a2, sizeof_MPGE3 # Point to next MR_MPRIM_GE3
	addi       $a3, $a3, sizeof_PGT3 # Point to next POLY_GT3
